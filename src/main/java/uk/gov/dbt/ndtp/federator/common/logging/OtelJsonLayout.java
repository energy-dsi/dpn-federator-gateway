 ...
 *
 * <p>Presence is determined by reading the MDC snapshot captured by the OTEL_MDC appender
 * (io.opentelemetry:opentelemetry-logback-mdc-1.0) at the original log-call site - NOT by
 * calling {@link Span#current()} directly at layout time. This layout runs on the
 * AsyncAppender's dedicated background worker thread (see logback.xml: the CONSOLE appender
 * using this layout is wrapped by ASYNC_CONSOLE), and OpenTelemetry's Context/current-span is a
 * ThreadLocal that does NOT propagate across that async thread hop - only the ILoggingEvent's
 * MDC snapshot (captured synchronously, on the original calling thread, before the event is
 * queued) survives it. Reading Span.current() here would always see the invalid root span,
 * regardless of whether the original call site was inside a properly makeCurrent()-scoped span.
 *
 * <p>This still mirrors the Python implementation's behaviour: "trace context is added when a
 * log is emitted inside an active span", checked via
 * {@code trace.get_current_span().get_span_context().is_valid} there, and via the OTEL_MDC
 * appender populating "trace_id"/"span_id"/"trace_flags" in MDC here (it performs the
 * equivalent Span.current()/SpanContext#isValid() check synchronously, before the async hop).
 *
...
public class OtelJsonLayout extends LayoutBase<ILoggingEvent> {
    ...
    public String doLayout(ILoggingEvent event) {
        ...
        // DSI EDIT: trace_id/span_id/trace_flags are TOP-LEVEL and conditional - read from the
        // MDC snapshot the OTEL_MDC appender (logback-mdc-1.0) captured synchronously at the
        // original log-call site, NOT from Span.current() at layout time (this layout runs on
        // ASYNC_CONSOLE's background worker thread, where Span.current()'s ThreadLocal context
        // never reflects the calling thread's makeCurrent() scope - see class Javadoc). All
        // three are added together or omitted together (matches
        // OTEL_LOG_TRACE_CONTEXT_EXPLAINED.md's "active span" rule).
        Map<String, String> mdcForTrace = event.getMDCPropertyMap();
        String traceId = mdcForTrace != null ? mdcForTrace.get("trace_id") : null;
        String spanId = mdcForTrace != null ? mdcForTrace.get("span_id") : null;
        if (traceId != null && spanId != null) {
            log.put("trace_id", traceId);
            log.put("span_id", spanId);
            log.put("trace_flags", mdcForTrace.getOrDefault("trace_flags", "00"));
        }
    ...
    /**
     * Looks for an "event.name" value on this log event - checking the SLF4J 2.x key-value pairs
     * first (the mechanism HeartbeatService uses via addKeyValue), then falling back to MDC (in
     * case a future call site sets it there instead, per the class Javadoc's extra={...}
     * pattern). Returns null if neither source carries it, in which case buildAttributes() falls
     * back to its original ordering (code.filepath first).
     */
    private static String findEventName(List<KeyValuePair> keyValuePairs, Map<String, String> mdc) {
        if (keyValuePairs != null) {
            for (KeyValuePair kv : keyValuePairs) {
                if ("event.name".equals(kv.key) && kv.value != null) {
                    return String.valueOf(kv.value);
                }
            }
        }
        if (mdc != null) {
            String fromMdc = mdc.get("event.name");
            if (fromMdc != null) {
                return fromMdc;
            }
        }
    ...
    private Map<String, Object> buildAttributes(ILoggingEvent event) {
        ...
        // DSI EDIT (event.name positioning - BUGFIX): the log-status-monitoring pipeline
        // (dpn-health-monitoring-service/data-prepper) flattens the OTLP attributes array by
        // FIXED POSITION and reads whatever lands at attributes[0] into log.event_name - it does
        // NOT match on the attribute's key name. Previously code.filepath/lineno/function/
        // logger.name were always written first, so attributes[0] was always code.filepath and
        // log.event_name silently ended up holding the source filename (e.g.
        // "HeartbeatService.java") instead of the real event name (e.g. "component.heartbeat"),
        // breaking the Component Health State dashboard panel's term filter on log.event_name.
        // Fix: when a log event carries an "event.name" key (via SLF4J 2.x
        // logger.atInfo().addKeyValue("event.name", ...) as HeartbeatService does, or via MDC),
        // emit it FIRST so it always occupies attributes[0], ahead of everything else. Log
        // events with no event.name are unaffected - attributes[0] falls back to code.filepath
        // exactly as before.
        String eventName = findEventName(keyValuePairs, mdc);
        if (eventName != null) {
        ...
        // event.name is skipped here since it was already placed first above (if present).
        if (keyValuePairs != null) {
            for (KeyValuePair kv : keyValuePairs) {
                if (!isReservedAttribute(kv.key) && !"event.name".equals(kv.key)) {
                    attributes.put(kv.key, kv.value);
                }
            }
        }

        // Optional custom attributes (e.g. dpn.participant_id) - set via MDC at the call site,
        // mirroring the Python services' extra={...} pattern. Any MDC key is passed through
        // verbatim as a custom attribute, EXCEPT:
        ...
        // - "event.name" - already placed first above (if present).
        if (mdc != null) {
            mdc.forEach((key, value) -> {
                if (!isReservedAttribute(key) && !"event.name".equals(key)) {
