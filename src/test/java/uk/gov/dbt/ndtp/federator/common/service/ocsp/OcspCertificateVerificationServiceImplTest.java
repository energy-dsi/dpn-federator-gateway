 ...
 *   {@code Mockito.mockStatic(...)}, so no TLS material or filesystem access is needed at all.</li>
 * </ul>
 *
 * <p>DSI EDIT (10 failing tests fix): {@code httpClientSupplier} in the production class is a
 * <b>lazy</b> {@code Supplier<HttpClient>} -
 * {@code () -> HttpClientFactoryUtils.createHttpClientWithMtls(properties)} - it is invoked fresh
 * on every single call to {@code checkCertificateStatus()}, not just once at construction time.
 * The original test scoped {@code mockStatic(HttpClientFactoryUtils.class)} to a
 * try-with-resources block that only wrapped the constructor call in {@code setUp()}. That mock
 * was closed long before any {@code @Test} method ran {@code verifyBeforeConnect()}, so every test
 * actually invoked the <b>real</b> {@code HttpClientFactoryUtils.createHttpClientWithMtls()},
 * which tried to load a real keystore/truststore from disk and threw
 * {@code FederatorSslException} - silently swallowed by {@code verifyBeforeConnect()} into
 * {@code NOT_FOUND} every time, regardless of what the mocked HTTP response said. The fix keeps
 * the static mock open for the entire test lifecycle (opened in {@code @BeforeEach}, closed in
 * {@code @AfterEach}) instead of just around construction, so it's still active whenever
 * {@code httpClientSupplier.get()} is actually invoked during the test body.
 */
...
class OcspCertificateVerificationServiceImplTest {
    ...
    // Kept open for the whole test (not just construction) because httpClientSupplier.get()
    // re-invokes HttpClientFactoryUtils.createHttpClientWithMtls(...) on every call, not just once.
    private MockedStatic<HttpClientFactoryUtils> httpClientFactoryMock;
    ...
    void setUp() throws Exception {
        ...
        httpClientFactoryMock = mockStatic(HttpClientFactoryUtils.class);
