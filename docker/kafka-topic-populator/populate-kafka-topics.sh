...
for newTopic in "${kafkaTopicsArray[@]}"; do
  kafka-topics --create --topic "$newTopic" --partitions 1 --replication-factor 1 --if-not-exists --bootstrap-server "$KAFKA_BROKER_SERVER" --command-config /certs/client-sasl.properties
...
for knowledgeTopic in "${knowledgeTopicsArray[@]}"; do
  ...
  kafka-topics --bootstrap-server "$KAFKA_BROKER_SERVER" --describe --topic "$knowledgeTopic" --command-config /certs/client-sasl.properties
  topic_exists=$?
  if [ ${topic_exists} -eq 0 ]; then
    echo "Loading knowledge topic data ${knowledgeDatasArray[${count}]} into topic ${knowledgeTopic}"
    kafka-console-producer --bootstrap-server "$KAFKA_BROKER_SERVER" --topic "$knowledgeTopic" --property parse.headers=true --property headers.separator="${DATA_DELIMITER}" --producer.config /certs/client-sasl.properties < ./input/knowledge/"${knowledgeDatasArray[${count}]}"
  else
