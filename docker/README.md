...
#### Kafka Authentication: SASL_OAUTHBEARER / PLAINTEXT / SSL

The gRPC solution's two Kafka clusters (`kafka-src`, `kafka-target`) support three client-facing auth modes. The mechanism is switched by **commenting/uncommenting a tagged block of properties** directly in each participating file - there is no env-var or `--env-file` toggle. Exactly one block per file must be uncommented at a time, and the choice must match across every file listed below.

- **SASL_OAUTHBEARER** (default) - clients authenticate with an OAuth2 access token (`client_credentials` grant) issued by a local Keycloak instance, and the brokers validate the token's signature against Keycloak's JWKS endpoint, over `SASL_SSL`. No custom broker plugin needed - this uses Apache Kafka's built-in OAuth support (KIP-768), already bundled in `cp-kafka:7.5.3`.
- **PLAINTEXT** - no TLS, no SASL at all on the client-facing listeners. Local/quick-testing only.
- **SSL** - plain TLS on the client-facing listeners, no SASL and no username/password/credentials of any kind. The broker doesn't require a client certificate (`KAFKA_SSL_CLIENT_AUTH: none`) - clients just need to trust the CA via a truststore.

A local `keycloak` service (realm `kafka`, client `kafka-client` / secret `kafka-client-secret`, auto-imported from [`docker-grpc-resources/keycloak/kafka-realm.json`](docker-grpc-resources/keycloak/kafka-realm.json), exposed at `http://localhost:8180`) always runs alongside the rest of the stack regardless of which mode is active - it's only actually used when the OAUTHBEARER block is uncommented.

**Files to edit, all in lockstep** (search each for `>>>>>>>>>> MECHANISM:` to find the tagged blocks):

| File | What it controls |
|---|---|
| [`docker-grpc-resources/docker-compose-shared.yml`](docker-grpc-resources/docker-compose-shared.yml) | `kafka-src` and `kafka-target` (broker listener security protocol + SASL config) **and** `kafka-ui` (both clusters' connection properties) - three blocks each |
| [`docker-grpc-resources/certs/client-sasl.properties`](docker-grpc-resources/certs/client-sasl.properties) | `kafka-topics-populator`'s `--command-config` |
| `tmp/federator-secrets-bcc/server-bcc.properties` | `federator-server`'s Kafka connection (`kafka.additional.*`) |
| `tmp/federator-secrets-bcc/client-bcc.properties` | `federator-client`'s Kafka connection (`kafka.additional.*`) |

Inter-broker traffic (the `INTERNAL` listener) always stays on plain SSL regardless of which mode is active - only the client-facing `EXTERNAL`/`SSL` listeners change.

After editing, recreate whichever containers changed:

```shell
docker compose --file docker/docker-compose-grpc.yml up -d
```

`up -d` is required (not `restart`) so Compose re-reads the changed files - `restart` reuses each container's already-applied config.
