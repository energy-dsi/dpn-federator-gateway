...
- Source and target Kafka brokers support three switchable authentication modes via a single `kafka.authMode` Helm value: `plaintext` (dev/test), `ssl` (TLS only, mTLS if client auth is required), and `sasl_ssl` (OAUTHBEARER over TLS, the production default).
- OAuth token validation for Kafka brokers and clients is performed against **Keycloak**, using a custom `OAuthBearerLoginCallbackHandler` with configurable client ID, JWKS URL, token endpoint, expected audience, and expected issuer.
- TLS keystore/truststore configuration (location, type, password, client-auth mode) is configurable per broker (source and target).
- Broker and Keycloak secrets (admin passwords, keystore/truststore passwords, OAuth client secret) are provisioned via Azure Key Vault using the CSI Secrets Store driver (`SecretProviderClass`), rather than being stored directly in Helm values.
- Kafka UI has been updated to authenticate against Keycloak using OAuth as well.
